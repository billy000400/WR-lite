...
void fit_ExpmCB(std::string filePath)
{
  ...
  c->SaveAs((plot_file_prefix+std::string(WRGenMean)+N_str+std::string(NGenMean)+suffix).c_str());
  c->Close();
}
...
RooExpmCB* ExpmCB_init(RooRealVar* rrv_x, double mean, std::string label)
{
 ...
 RooRealVar* rrv_alpha_CB = new RooRealVar((std::string("rrv_alpha_ExpmCB_")+label).c_str(), label.c_str(), 1, 0., 10.0);
