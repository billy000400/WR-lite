...
void testFit_ExpmCB(std::string filePath)
{
  ...
  //// Plot on frames
  // plot data on frames
  ds_WR_RecoMass_ee.plotOn(eeFrame_doubleCB, Binning(200), DataError(RooAbsData::SumW2));
  ds_WR_RecoMass_mumu.plotOn(mumuFrame_doubleCB, Binning(200), DataError(RooAbsData::SumW2));
...
RooExpmCB* ExpmCB_init(RooRealVar* rrv_x, double mean, std::string label)
{
 ...
 RooRealVar* rrv_beta_CB = new RooRealVar((std::string("rrv_beta_ExpmCB_")+label).c_str(), label.c_str(), 1., 1e-2, 5.);
 RooRealVar* rrv_m_CB = new RooRealVar((std::string("rrv_m_ExpmCB_")+label).c_str(), label.c_str(), 1., 0., 5.);
