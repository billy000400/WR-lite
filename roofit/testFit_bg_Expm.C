...
void testFit_bg_Expm()
{
  ...
  //// declare model
  // mumujj
  RooRealVar *a_mm = new RooRealVar("a_mm", "a_mm", -2e-1, -6e-1, -5e-2);
  ...
  // eejj
  RooRealVar *a_ee = new RooRealVar("a_ee", "a_ee", -2e-01, -6e-1, -5e-2);
  ...
  //// fit model
  RooFitResult *r_mm = model_mm->fitTo(ds_bg_mumujj, Save(), SumW2Error(kTRUE), Range(650, 2500), Strategy(1));
  RooFitResult *r_ee = model_ee->fitTo(ds_bg_eejj, Save(), SumW2Error(kTRUE), Range(650, 2500), Strategy(1));
