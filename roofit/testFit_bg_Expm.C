...
void testFit_bg_Expm()
{
  ...
  // eejj
  RooRealVar *a_ee = new RooRealVar("a_ee", "a_ee", -2.2e-01, -2.6e-1, -1e-1);
  RooRealVar *b_ee = new RooRealVar("b_ee", "b_ee", 6e-01, 1e-2, 6e-1);
  RooExpm *model_ee = new RooExpm("eejj bg from DY+ttbar ", "exponential bg", *eejjMass_bg, *a_ee, *b_ee);

  //// fit model
  RooFitResult *r_mm = model_mm->fitTo(ds_bg_mumujj, Save(), SumW2Error(kTRUE), Range(400, 3000), Strategy(2));
  RooFitResult *r_ee = model_ee->fitTo(ds_bg_eejj, Save(), SumW2Error(kTRUE), Range(400, 3000), Strategy(2));
