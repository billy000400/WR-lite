...
RooDSCBShape* DSCB_init(RooRealVar* rrv_x, double mean, std::string label)
{
 ...
 RooRealVar* rrv_sigma_CB = new RooRealVar((std::string("sigma")).c_str(), label.c_str(), 260, 50, 2000);
 RooRealVar* rrv_alpha_CB_I = new RooRealVar((std::string("alpha_left")).c_str(), label.c_str(), 1, 0., 20);
 RooRealVar* rrv_alpha_CB_II = new RooRealVar((std::string("alpha_right")).c_str(), label.c_str(), 1., 0., 20.);

 RooRealVar* rrv_n_CB_I = new RooRealVar((std::string("n_left")).c_str(), label.c_str(), 5, 0., 3000.);
 RooRealVar* rrv_n_CB_II = new RooRealVar((std::string("n_right")).c_str(), label.c_str(), 5, 0., 3000.);

 return new RooDSCBShape((std::string("DSCB_")+label).c_str(), label.c_str(), *rrv_x, *rrv_mean_CB,*rrv_sigma_CB,*rrv_alpha_CB_I,*rrv_alpha_CB_II,*rrv_n_CB_I,*rrv_n_CB_II);
}
