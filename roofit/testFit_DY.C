...
void testFit_DoubleCB()
{
  ...
  RooPlot *frame1 = WR_RecoMass->frame(Title("DY lljj Reco Mass (top 2 pT lepton)"));
  ds1.plotOn(frame1, Binning(128));
  RooPlot *frame2 = N_RecoMass_Match->frame(Title("DY ljj Mass, Reco by Resolved NN"));
  ds1.plotOn(frame2, Binning(128));
  RooPlot *frame3 = N_RecoMass_NN->frame(Title("DY ljj Mass, Reco by SuperResolved NN"));
