...
void extractTtbar(){
	...
	std::string files[1] = {"TTTo2L2Nu/out_TTTo2L2Nu_"};				
	const double xsec[1] = {88.29};
		
	double fileNumber = 592;
	...
	for(int j = 0; j<1; j++){
		...
		for(int i =0; i<twoDHistoRecoCount/2; i++){
			...
			print2DHistos(twoDhistogramsReco[twoDRecoMerge1[i]], c, "ttbar_", twoDhistoNamesReco2[twoDRecoMerge1[i]]);
		}
		...
		for(int i =0; i<oneDHistoGenCount; i++){
			print1DHistos(oneDhistogramsGen[i], c, "ttbar_", oneDhistoNamesGen[i]);
		}
		...
		for(int i =0; i<oneDHistoDivCount; i++){
			print1DHistos(oneDhistogramsDiv[i], c, "ttbar_", oneDhistoNamesDiv[i]);
		}
		
		for(int i =0; i<twoDHistoNNCount/2; i++){
			twoDhistogramsNN[twoDNNMerge1[i]]->Add(twoDhistogramsNN[twoDNNMerge2[i]]);
			print2DHistos(twoDhistogramsNN[twoDNNMerge1[i]], c, "ttbar_", twoDhistoNamesNN2[twoDNNMerge1[i]]);
		}
	}
}